...
foreach ($user in $BatchedUsers) {
    ...
    if ($MoveRequest -ne $null -and $MoveRequest.BatchName -eq $newBatchName) {
     ...
     if ($MoveRequest -ne $null -and $MoveRequest.Status -eq "Completed") {
        $result = "No change for $userPrimarySmtpAddress, already completed migration"
        $results += [PSCustomObject]@{ User = $userPrimarySmtpAddress; Action = 'None'; BatchName = "Completed"; Result = $result }
        continue # Skip to the next iteration
    }
    ...
    try {
        $moveRequestStatus = (Get-MoveRequest $userPrimarySmtpAddress).Status
        if ($moveRequestStatus -eq "Completed") {
            $result = "Move Request for $userPrimarySmtpAddress is already completed."
        } else {
            ...
            $result = "Updated Move Request for $userPrimarySmtpAddress to set complete."
        }
    } catch {
        $result = "Failed to set complete on $userPrimarySmtpAddress. Something is wrong with this user's migration; check it."
    }
...
$MoveStatsHTML = $MoveStats | Select-Object @{Name='DisplayName';Expression={$_.DisplayName}}, 
    ...
    @{Name='MailboxGUID';Expression={$_.ExchangeGuid}} | sort DisplayName |
...
$Results | Where-Object {$_.Result -like "*TargetUserAlreadyHasPrimaryMailboxException*"} | ForEach-Object {
    ...
    if ($item) {
        ...
        $fieldsToUpdate = @{"DataConsistencyScore" = $null; "PercentComplete" = "100"; "LargeItems" = $null; "BadItems" = $null}
        foreach ($field in $fieldsToUpdate.Keys) {
            Set-PnPListItem -List "OnPremMailboxes" -Identity $item.Id -Values @{$field = $fieldsToUpdate[$field]}
        }
    }
...
foreach ($moveRequest in $UpdatedMoveRequests) {
    if ($moveRequest.Status -eq "Completed") {
        ...
        $groupMembers = Get-ADGroupMember "Intune-EXO" | Select -ExpandProperty DistinguishedName
