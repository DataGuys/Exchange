...
$testEmailAddresses = "ghall@helient.com", "msgadmin@polsinelli.com" # Add more as needed
...
function Send-TminusEmail {
    ...
    foreach ($recipient in $recipients) {
        Send-MailMessage -SmtpServer $smtpServer -From $smtpFrom -To $recipient -Subject $Subject -Body $Body -BodyAsHtml
    }
}
...
# Determine days to migration
  ...
  foreach ($day in $daysToMigration) {
        ...
        # Send the email
           Send-MailMessage -SmtpServer $smtpServer -From $smtpFrom -To $testuser -Subject "Your migration is in $day days" -Body $emailBody -BodyAsHtml
    }
    }
    Exit
    }
...
foreach ($user in $batchedUsers) {
    ...
    if ($daysToMigration -ne $null) {
        ...
        if (Test-Path $emailTemplatePath) {
            ...
            Send-MailMessage -SmtpServer $smtpServer -From $smtpFrom -To $user.PrimarySmtpAddress -Subject "Your migration is in $daysToMigration days" -Body $emailBody -BodyAsHtml
        } else {
